...
get_duckpgq_source() {
    ...
    local duckpgq_ref="v1.4-andium"  # track upstream branch; avoid shallow/pinned commits missing on CI
...
verify_extensions() {
    ...
    if nm "$lib_path" 2>/dev/null | grep -q "DuckpgqExtension"; then
        log_info "  ✓ DuckPGQ extension found"
    else
        missing+=("duckpgq")
    fi
